...
export default function Gallery({ filtro = "home" }) { // Declara un componente Gallery con un prop filtro por defecto "home"
  ...
  return (
    <> {/* Fragmento de React */}
      {fotos
        ...
        .map((foto) => ( // Mapea las fotos filtradas para renderizarlas
          <Col key={foto.id}> {/* Col componente de react-bootstrap con una clave única */}
            <Card className='text-white'> {/* Card componente de react-bootstrap */}
              <Card.Img src={foto.src} alt={foto.alt} /> {/* Imagen de la tarjeta */}
              <Card.ImgOverlay> {/* Capa superpuesta de la tarjeta */}
                <Card.Text className='text-end'> {/* Texto en la esquina superior derecha */}
                  <IconHeart id={foto.id} /> {/* Icono de corazón con un ID de foto */}
                </Card.Text>
                <Card.Text>{foto.alt}</Card.Text> {/* Texto de la descripción de la foto */}
              </Card.ImgOverlay>
            </Card>
          </Col>
        ))}
